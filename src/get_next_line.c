...
{
	static char	buffer[1024][GNL_BUFFER + 1];
	char		*line;
	char		*newline;
	int			bytes;

	line = NULL;
	if (fd < 0 || fd >= 1024 || GNL_BUFFER <= 0)
		return (ft_memset(buffer[fd], 0, GNL_BUFFER + 1), NULL);
	while (1)
	{
		if (!buffer[fd][0])
		{
			bytes = read(fd, buffer[fd], GNL_BUFFER);
			if (bytes == 0)
				return (line);
			if (bytes < 0)
				return (ft_memset(buffer[fd], 0, GNL_BUFFER + 1), free(line), NULL);
		}
